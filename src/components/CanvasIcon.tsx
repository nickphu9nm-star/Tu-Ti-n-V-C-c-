...
export type IconType = 'heart' | 'sword' | 'shield' | 'zap' | 'hammer' | 'bag' | 'market' | 'book' | 'skull' | 'trophy' | 'crown' | 'medal' | 'clock' | 'gift' | 'send' | 'close' | 'swords' | 'coins' | 'settings' | 'sound_on' | 'sound_off' | 'logout' | 'trash' | 'users' | 'flag' | 'arrow_up' | 'message-square' | 'search' | 'info' | 'tower' | 'bell';
...
const CanvasIcon: React.FC<Props> = ({ type, size = 16, color = '#9ca3af', className }) => {
  ...
  useEffect(() => {
    ...
    switch (type) {
        ...
        case 'users': ctx.arc(cx-size*0.2, cy+size*0.1, size*0.2, 0, Math.PI*2); ctx.stroke(); ctx.arc(cx+size*0.2, cy-size*0.1, size*0.2, 0, Math.PI*2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(cx-size*0.4, cy+size*0.4); ctx.quadraticCurveTo(cx-size*0.2, cy+size*0.1, cx, cy+size*0.4); ctx.stroke(); break;
        case 'flag': ctx.moveTo(size*0.2, size*0.9); ctx.lineTo(size*0.2, size*0.1); ctx.lineTo(size*0.8, size*0.3); ctx.lineTo(size*0.2, size*0.5); ctx.stroke(); break;
        case 'arrow_up': ctx.moveTo(cx, size*0.8); ctx.lineTo(cx, size*0.2); ctx.stroke(); ctx.moveTo(size*0.3, size*0.4); ctx.lineTo(cx, size*0.2); ctx.lineTo(size*0.7, size*0.4); ctx.stroke(); break;
        case 'message-square': ctx.moveTo(size*0.2, size*0.2); ctx.lineTo(size*0.8, size*0.2); ctx.lineTo(size*0.8, size*0.7); ctx.lineTo(size*0.4, size*0.7); ctx.lineTo(size*0.2, size*0.9); ctx.lineTo(size*0.2, size*0.2); ctx.stroke(); break;
        case 'search': ctx.arc(cx-2, cy-2, size*0.3, 0, Math.PI*2); ctx.stroke(); ctx.moveTo(cx+size*0.1, cy+size*0.1); ctx.lineTo(cx+size*0.4, cy+size*0.4); ctx.stroke(); break;
        case 'info': ctx.arc(cx, cy, size*0.4, 0, Math.PI*2); ctx.stroke(); ctx.beginPath(); ctx.moveTo(cx, cy-size*0.1); ctx.lineTo(cx, cy+size*0.2); ctx.stroke(); ctx.beginPath(); ctx.arc(cx, cy-size*0.2, 0.5, 0, Math.PI*2); ctx.fill(); break;
        case 'tower': ctx.moveTo(size*0.3, size*0.9); ctx.lineTo(size*0.3, size*0.4); ctx.lineTo(size*0.5, size*0.2); ctx.lineTo(size*0.7, size*0.4); ctx.lineTo(size*0.7, size*0.9); ctx.stroke(); ctx.moveTo(size*0.2, size*0.9); ctx.lineTo(size*0.8, size*0.9); ctx.stroke(); ctx.moveTo(size*0.3, size*0.6); ctx.lineTo(size*0.7, size*0.6); ctx.stroke(); break;
        case 'bell': ctx.moveTo(cx, size*0.1); ctx.quadraticCurveTo(size*0.8, size*0.1, size*0.8, size*0.3); ctx.lineTo(size*0.9, size*0.7); ctx.lineTo(size*0.1, size*0.7); ctx.lineTo(size*0.2, size*0.3); ctx.quadraticCurveTo(size*0.2, size*0.1, cx, size*0.1); ctx.stroke(); ctx.beginPath(); ctx.arc(cx, size*0.7, size*0.1, 0, Math.PI); ctx.fill(); break;
