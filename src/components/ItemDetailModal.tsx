...
import { MARKET_BASE_VALUES } from '../constants';
...
const getRarityColor = (rarity: ItemRarity) => { switch (rarity) { case ItemRarity.RED: return 'text-red-500'; case ItemRarity.ORANGE: return 'text-orange-500'; case ItemRarity.GOLD: return 'text-yellow-400 font-bold'; case ItemRarity.PURPLE: return 'text-purple-400'; case ItemRarity.BLUE: return 'text-blue-400'; case ItemRarity.GREEN: return 'text-green-400'; default: return 'text-gray-400'; } };
...
const ItemDetailModal: React.FC<Props> = ({ item, onClose, onEquip, onUnequip, onListMarket, isEquipped }) => {
  ...
  const baseMarketValue = MARKET_BASE_VALUES[item.rarity] || 0;
        ...
        <div className="p-6">
          ...
          <div className="space-y-4 text-sm text-gray-200">
            ...
            {item.statLines && item.statLines.length > 0 && (
                <div className="bg-void-900/50 p-3 rounded-lg border border-void-700 space-y-2 max-h-[150px] overflow-y-auto">
                    {item.statLines.map((stat, idx) => {
                        ...
                        let label = ""; if (stat.code === 'hp') label = "Sinh Lực"; else if (stat.code === 'atk') label = "Công Kích"; else if (stat.code === 'def') label = "Phòng Thủ"; else if (stat.code === 'speed') label = "Tốc Độ"; else if (stat.code === 'hpPercent') label = "Sinh Lực %"; else if (stat.code === 'atkPercent') label = "Công Kích %"; else if (stat.code === 'defPercent') label = "Phòng Thủ %"; else if (stat.code === 'breakthroughChance') label = "Tỷ Lệ Đột Phá"; else if (stat.code === 'lifestealPercent') label = "Hút Máu"; else if (stat.code === 'cultivationSpeedPercent') label = "Tăng Tốc Tu Luyện";
            ...
            {item.embeddedStones && item.embeddedStones.length > 0 && (
                <div className="bg-void-900 p-3 rounded-lg border border-void-700">
                    <h4 className="text-xs font-bold text-gray-500 uppercase mb-2">Đá Đã Khảm ({item.embeddedStones.length}/5)</h4>
                    <div className="space-y-1">
                        {item.embeddedStones.map((stone, idx) => (
                            <div key={idx} className="flex justify-between items-center text-xs border-b border-void-800 last:border-0 pb-1">
                                <span className={getRarityColor(stone.rarity)}>{stone.name}</span>
                                <span className="text-gray-400">{stone.stats?.lifestealPercent ? `+${stone.stats.lifestealPercent}% Hút Máu` : `+${stone.stats?.cultivationSpeedPercent}% Tốc Độ`}</span>
                            </div>
                        ))}
                    </div>
                </div>
             ...
             <div className="flex flex-col gap-1">
                 <div className="flex justify-between items-center bg-void-900 px-3 py-2 rounded border border-void-700"><span className="text-gray-400 text-xs">Giá Cơ Bản (Phẩm chất)</span><span className="text-white font-mono font-bold">{baseMarketValue.toLocaleString()} <span className="text-gold-500">Linh Thạch</span></span></div>
                 <div className="flex justify-between items-center bg-void-900 px-3 py-2 rounded border border-void-700"><span className="text-gray-400 text-xs">Giá Trị Thực (Ước tính)</span><span className="text-white font-mono font-bold">{Math.floor(baseMarketValue * enhancementMultiplier).toLocaleString()} <span className="text-gold-500">Linh Thạch</span></span></div>
             </div>
          </div>
